...
call :log_message "INFO" "Build script started"
call :log_message "INFO" "Configuration: %CONFIG%"
call :log_message "INFO" "Platform: %PLATFORM%"

REM ========== START BUILD TIMER ==========
set "BUILD_START_TIME=%time%"

REM ========== VERIFY ENVIRONMENT ==========
call :log_message "INFO" "Verifying build environment..."
call :log_message "INFO" "Running Phase 2 enhanced dependency verification..."
...
if exist "%ROOT_DIR%\scripts\verify_dependencies.ps1" (
    ...
    if %ERRORLEVEL% NEQ 0 (
        call :log_message "ERROR" "Dependency verification failed"
        echo ERROR: Critical dependencies missing. See verification output above.
        exit /b 1
    )
    call :log_message "SUCCESS" "All dependencies verified successfully"
) else (
    call :log_message "WARNING" "Dependency verification script not found, running basic checks"
)

REM Check for Visual Studio environment
where cl >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    call :log_message "ERROR" "Visual C++ Compiler (cl.exe) not found."
    call :log_message "ERROR" "Please run this script from a Visual Studio Developer Command Prompt."
    call :log_message "ERROR" "You can find it in: Start Menu -> Visual Studio 20XX -> Developer Command Prompt"
    exit /b 1
)
...
where rc >nul 2>&1
if %ERRORLEVEL% NEQ 0 (
    call :log_message "ERROR" "Resource Compiler (rc.exe) not found."
    call :log_message "ERROR" "Please run this script from a Visual Studio Developer Command Prompt."
    exit /b 1
)
...
if "%SKIP_INSTALLER%"=="0" (
    ...
    if %ERRORLEVEL% NEQ 0 (
        call :log_message "WARNING" "NSIS Compiler (makensis.exe) not found."
        call :log_message "WARNING" "Installer will not be created."
        call :log_message "WARNING" "Please install NSIS from https://nsis.sourceforge.io/Download"
        set "SKIP_INSTALLER=1"
    )
)

REM ========== CREATE BUILD DIRECTORIES ==========
call :log_message "INFO" "Creating build directories..."
...
REM ========== PREBUILD STEPS ==========
call :log_message "INFO" "Running prebuild steps..."
...
REM ========== BUILD RESOURCES ==========
call :log_message "INFO" "Building resources..."
rc /nologo /d%CONFIG% /fo"%OBJ_DIR%\resource.res" "%RES_DIR%\resource.rc"
if %ERRORLEVEL% NEQ 0 (
    call :log_message "ERROR" "Resource compilation failed"
    exit /b 1
)

REM ========== COMPILE SOURCE FILES ==========
call :log_message "INFO" "Compiling source files..."
...
for %%F in (%SOURCE_FILES%) do (
    set "FILENAME=%%~nxF"
    call :log_message "INFO" "Compiling %%F..."
    cl %CFLAGS% "%%F"
    if !ERRORLEVEL! NEQ 0 (
        call :log_message "ERROR" "Failed to compile %%F"
        exit /b 1
    )
)

REM ========== LINK APPLICATION ==========
call :log_message "INFO" "Linking application..."
...
if %ERRORLEVEL% NEQ 0 (
    call :log_message "ERROR" "Linking failed"
    exit /b 1
)

REM ========== RUN TESTS ==========
if "%SKIP_TESTS%"=="0" (
    call :log_message "INFO" "Running tests..."
    
    if exist "%TEST_DIR%\run_tests.bat" (
        call "%TEST_DIR%\run_tests.bat" "%BIN_DIR%"
        if !ERRORLEVEL! NEQ 0 (
            call :log_message "ERROR" "Tests failed"
            exit /b 1
        )
    ) else (
        call :log_message "WARNING" "No test script found at %TEST_DIR%\run_tests.bat"
    )
) else (
    call :log_message "INFO" "Skipping tests as requested"
)

REM ========== COPY REQUIRED FILES ==========
call :log_message "INFO" "Copying required files to bin directory..."
xcopy /y "%RES_DIR%\folder_icon.png" "%BIN_DIR%\"
xcopy /y "%RES_DIR%\icon.ico" "%BIN_DIR%\"
xcopy /y "%ROOT_DIR%\LICENSE.txt" "%BIN_DIR%\"

REM ========== CREATE INSTALLER ==========
if "%SKIP_INSTALLER%"=="0" (
    call :log_message "INFO" "Creating installer..."
    
    REM Copy installer resources
    xcopy /y "%INSTALLER_DIR%\installer.nsi" "%BIN_DIR%\"
    xcopy /y "%RES_DIR%\installer_image.bmp" "%BIN_DIR%\"
    xcopy /y "%RES_DIR%\header.bmp" "%BIN_DIR%\"
    ...
    if %ERRORLEVEL% NEQ 0 (
        popd
        call :log_message "ERROR" "Installer creation failed"
        exit /b 1
    )
    popd
    
    REM Move installer to build directory
    move /y "%BIN_DIR%\%PROJECT_NAME%-Setup.exe" "%BUILD_DIR%\%PROJECT_NAME%-%VERSION_MAJOR%.%VERSION_MINOR%.%VERSION_PATCH%.%BUILD_NUMBER%-Setup.exe"
    
    call :log_message "INFO" "Installer created: %BUILD_DIR%\%PROJECT_NAME%-%VERSION_MAJOR%.%VERSION_MINOR%.%VERSION_PATCH%.%BUILD_NUMBER%-Setup.exe"
) else (
    call :log_message "INFO" "Skipping installer creation as requested"
)
...
REM ========== FINALIZE ==========
call :log_message "SUCCESS" "Build completed successfully in %BUILD_DURATION%!"
call :log_message "INFO" "Output executable: %BIN_DIR%\%PROJECT_NAME%.exe"

if "%SKIP_INSTALLER%"=="0" (
    call :log_message "INFO" "Installer: %BUILD_DIR%\%PROJECT_NAME%-%VERSION_MAJOR%.%VERSION_MINOR%.%VERSION_PATCH%.%BUILD_NUMBER%-Setup.exe"
)
